...
<li>
    <a href="{{ $isAdmin ? route('admin.orders.index') : route('orders.index') }}"
        class="nav-link {{ request()->routeIs('*.orders.*') || request()->routeIs('orders.*') ? 'nav-link-active' : '' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
<li>
    <a href="{{ $isAdmin ? route('admin.menus.index') : route('menus.index') }}"
        class="nav-link {{ request()->routeIs('*.menus.*') || request()->routeIs('menus.*') ? 'nav-link-active' : '' }}">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
